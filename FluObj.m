    ...
    methods
        function obj=FluObj(size,shape_type)
            if (size>0) && (isa(shape_type,'char'))
                obj.d=uint16(size); obj.shape=shape_type; % set the properties to a class instance
    ...
    methods
        ...
        function p=gauss(FluObj,xC,yC,x,y)
            if FluObj.shape == 'G';
                ...
                x=double(x); y=double(y); xC=double(xC); yC=double(yC); % necessity for further calculation
